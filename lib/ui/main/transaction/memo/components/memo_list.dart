...
class MemoList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return memoListState.when(
      data: (memoListViewModel) {
        final dailyMemoList = memoListViewModel?.dailyMemoListDTO ?? [];
                                  ...
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) => MemoEdit(
                                        memoId: memo.id, // 선택한 메모의 ID 전달
                                        userId: userId!, // 사용자 ID 전달
                                        title: memo.title, // 선택한 메모의 제목 전달
                                        content: memo.content, // 선택한 메모의 내용 전달
      ...
      error: (error, stack) => Center(child: Text('메모를 불러오는 중 오류가 발생했습니다.')),
    );
  }
}
